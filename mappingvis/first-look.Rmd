...
y <- read_sf(file.path(data_dir, "glaciers.geojson")) %>%
  ...
  filter(Latitude > 36.0, Latitude < 36.5) %>%
  filter(Longitude > 70.8, Longitude < 71.4)
  #filter(Sub_basin == "Dudh Koshi")
dim(y)
```

```{r}
x_train <- read_subset(vrt_path, st_bbox(y))
print(dim(x_train))
```

```{r}
writeRaster(x_train, file.path(data_dir, "LE07_152035_20060917-subset.tiff"), format = "GTiff", overwrite = TRUE)
```
