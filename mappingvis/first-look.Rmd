...
general spatial data manipulation. `RStoolbox` and `tmap` give useful functions
for spatial data visualization specifically.
...
```{r}
raw_dir <- file.path(params$data_dir, "raw")
...
for (i in seq_len(nrow(data_links))) {
  download.file(
    data_links$link[i], 
    file.path(raw_dir, data_links$name[i]),
    method = "libcurl",
    quiet = TRUE
  )
}
...
```{r}
x_paths <- dir(raw_dir, "*tiff", full.names = TRUE)
vrt_path <- file.path(raw_dir, "region.vrt")
...
```{r}
y <- read_sf(file.path(raw_dir, "glaciers.geojson")) %>%
